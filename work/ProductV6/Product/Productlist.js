...
function list(product2 = product1) {
    ...
    for (let p of product2) { //เปิด loop เพื่อเพิ่มlistอาหารอื่นๆ
        ...
        function alertAdd(event) {
            ...
            CookieUtil.set(`${p.nameEng}`, qty, Date(6000));
        ...
        function canCel() {
            ...
            cart.items.forEach((item) => {
                CookieUtil.unset(item.product.nameEng)
            })
            cart.items = [];
            cart.itemIdIncart = [];
            cart.totalPrice = 0;
...
function checkCookie(cart) {
    product.forEach((p) => {
        if (CookieUtil.get(p.nameEng) != null) {
            ...
            cart.items.push({ product: p, qty: parseInt(CookieUtil.get(p.nameEng)) })
            cart.totalQty += parseInt(CookieUtil.get(p.nameEng));

        }
    });
    cart.items.forEach((p) => {
        cart.totalPrice += p.product.price * p.qty;
    })
}